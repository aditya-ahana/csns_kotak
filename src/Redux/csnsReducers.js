...
let initialData = {
  availableReportTypes: ["PDF", "Excel"].sort(),
  requestPhases: ["Completed", "In-progress", "Failed"].sort(),
  rowOptions: [5, 10, 25, 40],
  requiredReportsData: [
...
export const slicedDataConfig = createSlice({
  ...
  reducers: {
    ...
    setRequestList: (state, action) => {
      handleReducerDispatch(state, action, state.requestList);
    },
    setRequestDetails: (state, action) => {
      handleReducerDispatch(state, action, state.requestDetails);
    },
  },
});
