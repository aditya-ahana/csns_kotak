export const Hindi = {
  // greeting: "नमस्ते, स्वागत है!",

  // sidenav
  dashboard: "डैशबोर्ड",
  createRequest: "अनुरोध बनाएँ",
  viewUpdateRequest: "अनुरोध देखें/अद्यतन करें",

  // dashboard
  noRequestInDashoard: "कोई अनुरोध नहीं है",

  // create request
  ticketNo: "टिकट संख्या",
  enterTicketNo: "टिकट नंबर दर्ज करें",
  ticketDesc: "टिकट विवरण",
  enterTicketDesc: "टिकट विवरण दर्ज करें",
  statementsReportRequire: "आपके लिए आवश्यक कथन/रिपोर्ट चुनें",
  selectedRequest: "चयनित अनुरोध",
  selectDetails: "विवरण चुनें",
  enterAccountNumber: "खाता संख्या दर्ज करें",
  enterPAN: "पैन दर्ज करें",
  enterCRN: "सीआरएन दर्ज करें",
  enterRRN: "आरआरएन दर्ज करें",
  enterAadhaarNumber: "आधार नंबर दर्ज करें",
  enterEmailID: "ईमेल आईडी दर्ज करें",
  enterCreditNumber: "क्रेडिट नंबर दर्ज करें",
  enterDebitNumber: "डेबिट नंबर दर्ज करें",
  enterMobileNumber: "मोबाइल नंबर दर्ज करें",
  from: "से",
  to: "तक",
  type: "प्रकार",
  amount: "राशि",
  enterAmount: "राशि डालें",
  date: "दिनांक",
  submit: "जमा करना",
  preview: "देखें",

  selectReports: "रिपोर्ट चुनें",
  only: "केवल",
  characters: "अक्षर",
  only10Characters: "केवल 10 अक्षर",
  ifNeededAmountMustBe: "यदि आवश्यक हो, तो राशि 1-6 अंक लंबी होनी चाहिए",
  ifNeededSelectDate: "यदि आवश्यक हो तो दिनांक चुनें",
  // only1060Characters: "केवल 10-60 अक्षर",

  // view request
  viewRequest: "अनुरोध देखें",
  searchByTicketRequester: "टिकट/अनुरोधकर्ता द्वारा खोजें",
  filter: "फ़िल्टर",
  ticketID: "टिकट आईडी",
  requests: "अनुरोध",
  status: "स्थिति",
  createdDate: "सृजित दिनांक",
  requester: "अनुरोधकर्ता",
  action: "कार्रवाई",
  inProgress: "प्रगति पर है",
  completed: "पूरा हो गया",
  failed: "असफल",
  details: "विवरण",
  eDraft: "ई ड्राफ्ट",
  rowsPerPage: "प्रति पृष्ठ पंक्तियाँ",

  // mail draft
  mailDraft: "मेल ड्राफ्ट",
  subject: "विषय",
  copyText: "कॉपी करें",

  // view request
  viewRequestDetails: "विवरण देखें",
  downloadAll: "सभी डाउनलोड करें",
  closeTicket: "टिकट बंद करें",
};
